...
do
  ...
  worker_public_ip=$(ssh -i "$privkeypath" -q -o "StrictHostKeyChecking=no" root@"$worker" 'hostname -I | tr " " "\n" | head -1')
  ssh -i "$privkeypath" -q -o "StrictHostKeyChecking=no" root@"$worker" "echo $token > /tmp/token.txt && curl -sfL https://get.k3s.io | sh -s - agent --token-file /tmp/token.txt --server https://${master_ip_priv}:6443 --node-external-ip ${worker_public_ip} --kubelet-arg=\"cloud-provider=external\" --kubelet-arg=\"provider-id=digitalocean://$worker_id\"" > /dev/null
done
